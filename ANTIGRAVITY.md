...
1. **Linear Gradients**:
   - Do **NOT** use `bg-gradient-to-*` (legacy syntax).
...
3. **Standard Spacing Scale ($px / 4$)**:
   ...
   - Divide pixels by 4 to map to standard Tailwind units:
...
1. **Clean Code**: Keep utility classes compliant with standard Tailwind CSS v4 to prevent build warnings. Use Tailwind utilities instead of writing raw inline styles or CSS rules.
